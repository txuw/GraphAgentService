# GraphAgentService 脚手架说明

本文档整理当前项目的整体结构、运行方式与扩展约定，作为继续扩展 Graph、LLM、Tools 与 API 的统一参考。

## 1. 项目定位

GraphAgentService 是一个基于 `FastAPI + LangGraph` 的服务端项目，重点提供：
...
- `src/graphagentservice/graphs/` 顶层只放 graph 共享基础设施，如 `registry.py`、`runtime.py`
...
```text
src/graphagentservice/graphs/<graph_name>/
├── __init__.py
├── builder.py
├── state.py
├── nodes.py
...
文件位置：`src/graphagentservice/llm/profile.py`
...
文件位置：`src/graphagentservice/llm/router.py`
...
文件位置：`src/graphagentservice/llm/factory.py`
...
文件位置：`src/graphagentservice/graphs/runtime.py`
...
`src/graphagentservice/common/config.py` 只负责三件事：
...
当前 `tool-agent` 位于 `src/graphagentservice/graphs/tool_agent/`，采用标准 LangGraph agent 回路：
...
默认工具集来自 `graphagentservice.tools.build_toolset()`，当前包含：
