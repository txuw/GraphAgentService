...
当前内置的 tool graph 采用一个很标准的 LangGraph agent 回路：
...
- `agent` 通过 `runtime.context.tool_model(binding="agent", tools=...)` 拿到绑定了工具的 ChatModel
- `tools` 节点直接复用 `ToolNode`
- `tools_condition` 决定是进入 `tools`，还是直接结束到 `finalize`
- `finalize` 从完整消息历史里提取最终答案和工具调用轨迹
...
这样每一轮模型消息和工具消息都会自动追加到 state 中，最终可以统一用于：

- 继续下一轮 agent 推理
- 生成 `answer`
- 生成 `tools_used`

默认工具集由 `build_toolset()` 提供，当前包含：

- `lookup_weather`
- `lookup_local_time`
- `calculate`

如果后续要新增工具，优先改 `toolset.py` 或把可复用工具提到共享 `tools/` 目录，不要把工具实现散落到 route 或 service 层。
...
HTTP 返回统一包一层 `GraphInvokeResponse`：
...
当前内置两个 graph 的 `stream_modes` 都是：
...
当前项目对前端暴露了一套单独的 SSE 协议层，用于把 LangGraph 原生流式事件转换为更稳定的聊天事件：

1. 前端先调用 `GET /api/sse/connect`
2. route 通过 `SseConnectionRegistry` 注册连接
3. 连接建立后先推送 `connected`
4. 前端再调用 `POST /api/chat/execute`
5. `ChatStreamService` 调用 `GraphService.stream_events(...)`
6. `SseEventAdapter` 把内部事件转换成前端协议事件
7. `SseConnectionRegistry` 把事件推送到对应连接
...
- 前端不直接感知 LangGraph 的 `updates/messages/result`
- 过程事件和正文 token 分开消费
- Convert 尽量停留在接口层与 service 适配层，不侵入 graph/node 内部逻辑
